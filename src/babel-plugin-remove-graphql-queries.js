...
const murmurhash =
    typeof murmurModule === 'function' ? murmurModule : murmurModule.murmurhash
...
function getTagImport(tag) {
    ...
    if (
        ...
        path.get(`init.callee`).isIdentifier({
            name: `require`
        }) &&
        ...
        visitor: {
            Program(path, state) {
                ...
                const nestedHookVisitor = {
                    CallExpression(path2) {
                        if (
                            ...
                            if (
                                this.templatePath.parentPath.isVariableDeclarator()
                            ) {
                ...
                const setImportForStaticQuery = templatePath => {
                    ...
                    while (
                        parent &&
                        ![`Program`, `JSXOpeningElement`].includes(
                            parent.node.type
                        )
                    ) {
                ...
                path.traverse({
                    JSXElement(jsxElementPath) {
                        if (
                            jsxElementPath.node.openingElement.name.name !==
                            `StaticQuery`
                        ) {
                            return
                        }
                ...
                function followVariableDeclarations(binding) {
                    ...
                    const node =
                        (_binding$path = binding.path) === null ||
                        _binding$path === void 0
                            ? void 0
                            : _binding$path.node
